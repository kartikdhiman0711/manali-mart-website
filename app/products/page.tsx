...
import { ChevronDown, ChevronRight, Star, Tag, Grid3x3, List, Search, ChevronLeft } from 'lucide-react';
...
export default function Products() {
  ...
  const handleCategorySelect = (categoryId: string, event: React.MouseEvent) => {
    ...
    if (expandedCategories.includes(categoryId)) {
      ...
      // If it was also selected, clear the selection to show all products
      if (selectedCategory === categoryId) {
        setSelectedCategory(null);
        setSelectedSubcategory(null);
      }
    } else {
      // Expand this category and close any others (only one can be open)
                      ...
                      <div className="bg-gray-50">
                        {category.subcategories.map((subcategory) => (
                          <button
                            key={subcategory.id}
                            type="button"
                            onClick={(e) => handleSubcategorySelect(subcategory.id, category.id, e)}
